...
class App extends Component {
  ...
  onChangeSearchInput = event => {
    this.setState({searchInput: event.target.value}, this.getFilteredList)
  }

  getFilteredList = () => {
    const {searchInput} = this.state
    const filteredList = initialTracksList.filter(eachTrack =>
  ...
  onDeleteTrack = id => {
    this.setState(prevState => ({
      trackList: prevState.trackList.filter(eachTrack => eachTrack.id !== id),
    }))
  }
  ...
  render() {
    ...
    return (
      <>
        ...
        <BottomContainer>
          <FlexContainer>
            <SongsPlaylistTitle>Songs Playlist</SongsPlaylistTitle>
            <InputContainer>
              <InputElement
                placeholder="Search"
